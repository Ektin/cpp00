...
/*   Updated: 2024/07/17 14:47:33 by EktinOpUrims     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
__int64_t	Fixed::compute_ten_power( void )
{
	...
	for (res = 1, cnt = 0; cnt < _frac_bits_cnt; ++cnt, res *= 10);
	return (res);
}
...
void	Fixed::display( void ) const
{
	std::cout << (this->getRawBits() >> get_frac_bits_cnt());

	std::cout << ".";

	std::cout << std::setw(this->get_frac_bits_cnt()) \
		<< std::setfill('0') \
	<< (__int64_t)(this->getRawBits() & (this->get_two_power() - 1)) * \
	this->get_ten_power() / this->get_two_power();
